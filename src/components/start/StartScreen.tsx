...
import { sampleProjectJson } from '../../data/sampleProject';
      ...
      <Shell>
        ...
        <div className="start-pane">
          ...
          <div className="import-panel">
            ...
            {/*
              The sample is generated by `sampleProjectJson()` from real scenes
              through the real serialiser, so it cannot be invalid and cannot
              drift from the schema. Offered both ways on purpose: copying is
              for taking the shape somewhere else — a script, an LLM prompt —
              and loading is for seeing it render in the next ten seconds.
            */}
            <div className="import-sample">
              <span className="import-or">or paste the JSON</span>
              <span className="spacer" />
              <button
                type="button"
                className="btn tiny"
                onClick={() => {
                  const sample = sampleProjectJson();
                  navigator.clipboard?.writeText(sample).then(
                    () => setCopied(true),
                    // Clipboard can be blocked; putting it in the box is a
                    // strictly better failure than a silent no-op.
                    () => setText(sample),
                  );
                  window.setTimeout(() => setCopied(false), 1600);
                }}
                title="A complete, valid example project — text, composition, and motion graphics"
              >
              ...
              <button
                type="button"
                className="btn tiny"
                onClick={() => setText(sampleProjectJson())}
                title="Put the sample in the box below"
              >
                Load sample
              </button>
            </div>
